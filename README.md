...
- **麻雀**: 既定は半荘ごとのポイント（±）を直接入れる形。回の合計が 0 になるかを確認する。
  「詳細を編集」で素点入力に切り替えると、返し点との差・ウマ・オカからポイントを計算する
  （千点単位に五捨六入した整数。端数のずれは1位に寄せて回の合計を 0 にする）。
  配給原点・返し点・ウマ（なし／5-10／10-20／10-30／20-30 から選ぶ）もそこで変えられる。麻雀は4人必須。
