...
- **麻雀**: 半荘ごとの「ウマ・オカを足す前のポイント」（素点と返し点の差を千点単位にした値）を入れると、
  順位ウマとオカを足した最終ポイントを表に出す。カッコの中が入れた点、大きい数字が最終ポイント。
  ウマ・オカ前の合計は −オカ（25,000点持ち30,000点返しなら −20pt）になるはずなので、そこも確認する。
  配給原点・返し点・ウマ（なし／5-10／10-20／10-30／20-30 から選ぶ）は「詳細を編集」で変えられる。
  ウマ・オカの計算を外せば、入れた点をそのまま使う。麻雀は4人必須。
